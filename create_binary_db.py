...
gulp_file = GulpVideoIO(args.output_folder+'/train_data.bin', 'wb', args.output_folder+'/train_meta.pkl')
gulp_file.open()
label_count = 0
counter = 0
for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
	...
	end_t = row.time_end
	folder_name = os.path.join(args.frames_path, label, video_id)+"_{:06d}_{:06d}".format(start_t, end_t)
