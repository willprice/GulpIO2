...
def optical_flow(imgs):
    ...
    img_opt_flow = np.zeros_like(frame1)
    img_opt_flow[..., 2] = 0

    # set a matrix to keep average optical flow for last k frames
    ...
    for img in imgs[1:]:
        ...
        if avg_count == num_avg_frames:
            ...
            img_opt_flow[..., 0] = cv2.normalize(flow[..., 0], None, 0, 255, cv2.NORM_MINMAX)
            img_opt_flow[..., 1] = cv2.normalize(flow[..., 1], None, 0, 255, cv2.NORM_MINMAX)
            # rgb = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)

            out_imgs.append(img_opt_flow)
            save_count += 1
        prvs = next
    return out_imgs
...
if __name__ == '__main__':
    ...
    create_chunk(inputs[0], args.shm_dir_path, args.is_test)
